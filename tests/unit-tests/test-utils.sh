...
unset HOME
export HOME
HOME=$(TMPDIR=/tmp mktemp -d -t buava-user-home.XXXXXXX)
...
function test_error(){
    ...
    local expected
    expected=$(echo -e "\033[0;31mTest\033[0m")
...
function test_warn(){
    ...
    local expected
    expected=$(echo -e "\033[0;33mTest\033[0m")
    assertEquals "$expected" "$(cat "$STDERRF")"
}
...
function test_info(){
    ...
    local expected
    expected=$(echo -e "\033[0;36mTest\033[0m")
...
function test_choose(){
    local res
    res=$(echo "" | choose "Which color?" "Red" "Yellow" "Green")
    assertEquals "Red" "$res"
    local res
    res=$(echo "0" | choose "Which color?" "Red" "Yellow" "Green")
    assertEquals "Yellow" "$res"
    local res
    res=$(echo -e "NoColor\n0" | choose "Which color?" "Red" "Yellow" "Green" 2> /dev/null)
    assertEquals "Yellow" "$res"
    local res
    res=$(echo -e "NoColor" | choose "Which color?" "Red" "Yellow" "Green" 2> /dev/null)
    assertEquals "Red" "$res"
    local res
...
function test_input(){
    local res
    res=$(echo "Red" | input "Which color?" "Yellow")
    assertEquals "Red" "$res"
    local res
...
function test_setup_configuration(){
    ...
    assertEquals 0 $?
    assertEquals "unapply_conf" "$(grep -v "Setup configuration for" "$STDOUTF")"
    ...
    echo "0" | assertCommandSuccess setup_configuration "$conf_file_path" \
        new_conf apply_conf unapply_conf
    assertEquals "$(echo -e "new_conf\napply_conf")" "$(grep -v "Setup configuration for" "$STDOUTF")"
    ...
    echo -e "\n" | assertCommandSuccess setup_configuration "$conf_file_path" \
        new_conf apply_conf unapply_conf
    assertEquals "apply_conf" "$(grep -v "Setup configuration for" "$STDOUTF")"
...
test_backup_no_backup_allowed() {
    ...
    local backup_files
    backup_files=("$HOME"/original_file.backup.*)
    [[ ! -e ${backup_files[0]} ]]
    assertEquals 0 $?
}
